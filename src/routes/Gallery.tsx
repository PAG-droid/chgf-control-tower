...
const photos: Photo[] = Array.isArray(gallery.photos) ? gallery.photos : []

function src(file: string): string {
  // Files live in public/gallery/, so BASE_URL keeps this correct on both
  // GitHub Pages (served from a subpath) and Azure (served from root).
      ...
      {(photo.caption || photo.credit || photo.time) && (
        <div className="mt-4 max-w-2xl text-center">
          ...
          <p className="mt-1 text-xs text-ink-500">
            {[photo.credit, photo.time].filter(Boolean).join(' · ')}
          </p>
        </div>
      )}
...
export default function Gallery() {
  ...
  const prev = useCallback(() => setOpen((i) => (i === null ? null : (i - 1 + photos.length) % photos.length)), [])
  const next = useCallback(() => setOpen((i) => (i === null ? null : (i + 1) % photos.length)), [])

  return (
    <div className="space-y-7">
      <header>
        <h1 className="text-3xl font-bold tracking-tight text-ink-100">{gallery.title}</h1>
        <p className="mt-2 max-w-2xl text-ink-400">
          {photos.length > 0
            ? `${photos.length} moment${photos.length === 1 ? '' : 's'} from the day.`
            : 'Nothing here yet.'}
        </p>
      </header>
      ...
      {photos.length === 0 ? (
        <div className="rounded-xl border border-navy-800 bg-navy-900/50 px-6 py-16 text-center">
          <p className="text-lg font-semibold text-ink-300">No photos yet</p>
          <p className="mx-auto mt-2 max-w-md text-sm leading-relaxed text-ink-500">
            Drop image files into <code className="rounded bg-navy-800 px-1.5 py-0.5 text-ink-400">public/gallery/</code>{' '}
            and list them in{' '}
            <code className="rounded bg-navy-800 px-1.5 py-0.5 text-ink-400">src/data/gallery.json</code>. They appear
      ...
      {open !== null && photos[open] && (
        <Lightbox photo={photos[open]} onClose={() => setOpen(null)} onPrev={prev} onNext={next} />
      )}
    </div>
  )
}
